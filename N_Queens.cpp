             ...
             vector<int>& upperBound, vector<int>&lowerBound, int n)
  {
    ...
    for(int row=0;row<n;row++)
    {
      if(leftRow[row]==0 && lowerBound[row+col]==0 && upperBound[n-1+ col-row]==0)
      {
        ...
        solve(col+1,board,ans,leftRow,upperBound,lowerBound,n);
        board[row][col]='.';
        leftRow[row]=0;
...
public:
    vector<vector<string>> solveNQueens(int n) {
      ...
      solve(0,board,ans,leftRow,upperBound,lowerBound,n);
      return ans;
    }
};
